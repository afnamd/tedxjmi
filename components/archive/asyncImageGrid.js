...
const AsyncImageGrid = ({galleryId}) =>{
    ...
    useEffect(()=>{        
        fetch(`https://www.flickr.com/services/rest/?method=flickr.galleries.getPhotos&api_key=8f7e34cc57c58c6e98d6378955b17fab&gallery_id=${galleryId}&format=json&nojsoncallback=1`)
        .then(res=>res.json())
            ...
            <div className="flex justify-center gap-1">
                {
                    Array(gridCount).fill(0).map((_,i)=>{
                        return (
                            <div key={i.toString()} className="flex flex-col gap-1 w-full md:w-auto ">
                                {
                                    [...data].map((gallery,j)=>{
                                    return j%gridCount===i?(
                                        <div key={gallery.displayName} className=" flex flex-col rounded-md w-full md:w-44  overflow-hidden ">
                                            <img src={`https://farm${gallery.farm}.staticflickr.com/${gallery.server}/${gallery.id}_${gallery.secret}_m.jpg`} onClick={(e) => {showImage(e,gallery)}}/>
                                        </div>
                                    ):null
                                    })
                                }
